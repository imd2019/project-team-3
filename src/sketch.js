...
window.addEventListener("addAction", (ev) => {
  window.dispatchEvent(
    new CustomEvent(ev.detail.name, { detail: ev.detail.data })
  );
...
window.addEventListener("joinDemo", (ev) => {
  if (ev.detail === "demo") {
    ...
    console.log("demoJoined");
  } else {
    player.changeParameters(-1, 1, 0);
    console.log("counterdemoJoined");
  }
});

window.addEventListener("watchDemo", () => {
  player.changeParameters(-1, 0, 0);
  console.log("demo watched");
});
...
window.addEventListener("statementDefended", (ev) => {
  if (ev.detail) {
    if (player.actionDone("coffeeHouse", "proDemo", true)) {
      player.changeParameters(1, -1, 1);
      console.log("statementDefendend" + "proDemo");
    } else {
      player.changeParameters(-1, 1, -1);
      console.log("statementDefendend" + "proCounterdemo");
    }
  } else if (player.actionDone("coffeeHouse", "proDemo", true)) {
    console.log("statementRevoked" + "proDemo");
    player.changeParameters(1, -1, -1);
  } else {
    console.log("statementRevoked" + "proCounterdemo");
    player.changeParameters(1, -1, -1);
  }
});
...
window.addEventListener("buyNewspaper", (ev) => {
  switch (ev.detail) {
    case "conspiracy-theorist":
      player.changeParameters(1, -1, -1);
      console.log("newspaper conspiracy");
      break;
    case "wannabe-influencer":
      player.changeParameters(1, -1, 1);
      console.log("newspaper wannabe-influencer");
      break;
    case "reflective-user":
      player.changeParameters(-1, 1, -1);
      console.log("newspaper reflective user");
      break;
    case "follower":
      player.changeParameters(-1, -1, -1);
      console.log("newspaper follower");
  }
});
...
function draw() {
  ...
  console.log(
    player.getPersona(),
    "M: " + player.manipulativity,
    "R: " + player.reflectivity,
    "A: " + player.attentionseeking
  );
}
window.draw = draw;
