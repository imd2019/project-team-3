...
let barLinkImg, coffeeHouseLinkImg, demoLinkImg, kioskLinkImg, parkLinkImg_kiosk, parkLinkImg_demo, parkLinkImg_coffeeHouse;
...
function preload() {
  ...
  demoLinkImg = loadImage("../img/park/4_interactionSpaces/4_demo.png");
...
function setupGame () {
  ...
  let box = new Sprite(30, 30, 3000, 2000);
  park.addChild(box);

  let demoLink = new DemoLink(1911, 325, 248, 166, demoLinkImg); // adjust position & size!
  box.addChild(demoLink);
  ...
  let parkForegnd = new InteractiveObject(2, 228, 3904, 543, parkForegndImg); // adjust position!
  ...
  let barLink = new BarLink(1092, 137, 418, 648, barLinkImg);
  demo.addChild(barLink);

  let kioskLink = new KioskLink(0, 0, 0, 0, kioskLinkImg); // adjust position & size!
  park.addChild(kioskLink);

  let parkLink_kiosk = new ParkLink(1480, 189, 184, 444, parkLinkImg_kiosk);
  kiosk.addChild(parkLink_kiosk);

  let parkLink_demo = new ParkLink(1612, 337, 523, 1159, parkLinkImg_demo);
  ...
  let demoSign = new DemoSign(790, 668, 329, 143, demoSignImg);
